...
{
     ros::init(argc, argv, "turtlebot_cf_tracking");
     ros::NodeHandle nh;
     cv::namedWindow("Draw Bounding Box",0);
     //cv::namedWindow("turtlebot_image");
    //  cv::namedWindow("turtlebot_depthimage");
    //  cv::namedWindow("KCFcpp");
     cv::startWindowThread();

  image_transport::ImageTransport it(nh); 
  image_transport::Subscriber sub_rgb= it.subscribe("/rgb/image_raw", 1, rgbimageCallback);
  image_transport::Subscriber sub_depth = it.subscribe("/depth/image_raw", 1, depthimageCallback);
  ros::Publisher BoundingBox_pub = nh.advertise<turtlebot_cf_tracking::BoundingBox>("BoundingBox",100);
 // ros::Rate loop_rate(10);
  turtlebot_cf_tracking::BoundingBox msg;
